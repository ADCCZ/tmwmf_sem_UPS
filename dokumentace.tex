...
\node[state, right=of conn] (auth) {AUTHENTICATED};
\node[state, below=of auth] (lobby) {IN\_LOBBY};
\node[state, left=of lobby] (room) {IN\_ROOM};
\node[state, left=of room] (game) {IN\_GAME};
\node[state, below=3cm of game] (short) {SHORT\_DISC};
\node[final, below=3cm of lobby] (long) {LONG\_DISC};

% Přechody
\draw[->] (disc) -- node[above] {\tiny connect()} (conn);
\draw[->] (conn) -- node[above] {\tiny HELLO} (auth);
\draw[->] (auth) -- node[right] {\tiny WELCOME} (lobby);
\draw[->] (lobby) -- node[above] {\tiny JOIN\_ROOM} (room);
\draw[->] (room) -- node[above] {\tiny GAME\_START} (game);

% Reconnect
\draw[->] (game) -- node[left, align=center] {\tiny timeout\\výpadek} (short);
\draw[->] (short) -- node[below, align=center] {\tiny RECONNECT\\+ GAME\_STATE} (game);
\draw[->] (short) -- node[right] {\tiny >90s} (long);

% Návrat do lobby
\draw[->] (game) to[bend right=45] node[below] {\tiny GAME\_END} (lobby);
\draw[->] (room) to[bend left=30] node[above] {\tiny LEAVE\_ROOM} (lobby);

% Odpojení
\draw[->] (lobby) to[bend left=60] node[above, align=center] {\tiny disconnect()\\QUIT} (disc);
\draw[->] (game) to[bend right=80] node[below, align=center] {\tiny user\\disconnect} (disc);

% List rooms loop
\draw[->] (lobby) to[loop above] node[above] {\tiny LIST\_ROOMS} (lobby);
